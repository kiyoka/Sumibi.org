  ...
  '(
    (*section
     ...
     (*ja
      (p
       (subsection
	(@ (title "このサイトは何をするサイトですか？"))
	(p
	 "『手ぶらで海外に行きたい、でもメールも送りたい』を実現するためのサイトです。")
	(p
	 "Sumibi.org (炭火.org) は海外の日本語入力できないパソコンからでも、日本語でメールやブログを書いたりできる無料サイトです。"
	 "海外のインターネットカフェ等の日本語入力(IME)の入っていない英語版Windowsなどから簡単に日本語入力できます。")
	(p
	 "海外からYahooやGoogleで『sumibi』と検索すると、このサイトが出てきます。海外から日本語でメールを送りたい時はWebメールサービスに入っておく"
